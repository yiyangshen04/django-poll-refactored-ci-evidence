...
def polls_edit(request, poll_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "Poll Updated successfully.", extra_tags=SUCCESS_TAGS
            )
            return redirect("polls:list")
...
def polls_delete(request, poll_id):
    ...
    if request.user != poll.owner:
        return redirect("home")
    poll.delete()
    messages.success(request, "Poll Deleted successfully.", extra_tags=SUCCESS_TAGS)
    return redirect("polls:list")
...
def add_choice(request, poll_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_choice.save()
            messages.success(
                request, "Choice added successfully.", extra_tags=SUCCESS_TAGS
            )
...
def choice_edit(request, choice_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_choice.save()
            messages.success(
                request, "Choice Updated successfully.", extra_tags=SUCCESS_TAGS
            )
            return redirect("polls:edit", poll.id)
    else:
...
def choice_delete(request, choice_id):
    ...
    if request.user != poll.owner:
        return redirect("home")
    choice.delete()
    messages.success(request, "Choice Deleted successfully.", extra_tags=SUCCESS_TAGS)
    return redirect("polls:edit", poll.id)
...
def poll_vote(request, poll_id):
    ...
    if not poll.user_can_vote(request.user):
        messages.error(request, "You already voted this poll!", extra_tags=WARNING_TAGS)
        return redirect("polls:list")
    ...
    if choice_id:
        ...
        messages.error(request, "No choice selected!", extra_tags=WARNING_TAGS)
